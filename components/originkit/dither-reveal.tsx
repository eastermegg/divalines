...
const DEFAULTS = {
  ...
  waveSpeed: 82,
  waveDensity: 25,
  // Ambient reveal: when the pointer is idle, drift a faint reveal
  // around on its own so the effect announces itself. Off by default.
  autoReveal: false,
  autoRevealStrength: 0.35,
};
...
interface DitherRevealProps {
  ...
  waveDensity?: number;
  autoReveal?: boolean;
  autoRevealStrength?: number;
  // Freeze the render loop (skips all GPU work) while keeping the last
  // frame on screen — e.g. for an off-screen / crossfaded-out layer.
  paused?: boolean;
  style?: CSSProperties;
}
...
function settings(p: DitherRevealProps) {
  ...
  return {
    ...
    autoReveal: p?.autoReveal ?? DEFAULTS.autoReveal,
    autoStrength: clamp(p?.autoRevealStrength, 0, 1, DEFAULTS.autoRevealStrength),
  };
}
...
export default function DitherReveal(props: DitherRevealProps) {
  const {
    ...
    autoReveal = DEFAULTS.autoReveal,
    autoRevealStrength = DEFAULTS.autoRevealStrength,
    paused = false,
    style,
  } = props;
  ...
  useEffect(() => {
    ...
    function resize() {
      // Cap DPR hard: the effect is a chunky halftone, so extra device
      // pixels are wasted GPU. 1.5 keeps it crisp at a fraction of the fill.
    ...
    let raf = 0;
    let lastDraw = 0;
    const minFrameMs = 1000 / 40; // cap at ~40fps — plenty for slow drift
    function render() {
      raf = requestAnimationFrame(render);
      const L = liveRef.current;
      // Skip all GPU work when off-screen or frozen — the canvas keeps
      // its last frame, so a paused/crossfaded-out layer costs nothing.
      ...
      // Reveal driver. A real pointer always wins at full strength; when
      // it's idle and autoReveal is on, a faint reveal drifts a slow
      // Lissajous path so the effect quietly announces itself. The
      // position is eased (not snapped) so pointer↔ambient handoff never
      // jumps, and `hot` bypasses the entered-gate for the ambient case.
      const pointerHot = mouse.target > 0.5;
      let activeTarget = mouse.target;
      let hot = mouse.entered;
      if (!pointerHot && L.autoReveal) {
        // Breathe the strength between ~55% and 100% of autoStrength —
        // the gentle pulse is what actually catches the eye, so the
        // reveal can stay faint yet still read as "interactive".
        const breathe = 0.78 + 0.22 * Math.sin(t * 0.9);
        activeTarget = L.autoStrength * breathe;
        hot = true;
        const driftX = 0.5 + 0.26 * Math.sin(t * 0.31);
        const driftY = 0.5 + 0.18 * Math.cos(t * 0.23);
        mouse.x += (driftX - mouse.x) * 0.04;
        mouse.y += (driftY - mouse.y) * 0.04;
      }
      mouse.active += (activeTarget - mouse.active) * 0.08;

      gl!.uniform1f(uTime, t);
      gl!.uniform2f(uMouse, mouse.x, mouse.y);
      gl!.uniform1f(uMouseActive, hot ? mouse.active : 0);
...
void main() {
    ...
    // Lift shadows before dithering so dark silhouettes still read as a
    // visible halftone instead of collapsing to near-black. Hue still
    // ramps off the ORIGINAL luminance, so the body stays violet.
    float litGray = pow(gray, 0.5);
    float tone = ditherTone(litGray, ps);
    // Tint the halftone: dots ramp from violet in the shadows to orange
    // in the highlights (brand heat), keeping the dot structure via tone.
    vec3 ditherPurple = vec3(0.431, 0.169, 0.659);
    vec3 ditherOrange = vec3(1.0, 0.478, 0.184);
    vec3 ditherColor = mix(ditherPurple, ditherOrange, smoothstep(0.12, 0.6, gray)) * tone;
