...
export type GradientOrbConfig = {
  /**
   * CSS color string for the canvas background, or "transparent" to
   * composite the orb over whatever sits behind the canvas.
   * @default "#0a0a0a"
   */
  background?: string;
...
export function GradientOrb({
  ...
  // The fragment shader already outputs premultiplied alpha, so a
  // transparent canvas composites cleanly over the page behind it.
  const isTransparent = config.background === "transparent";

  return (
    <div
      className={`w-full h-full ${className}`}
      style={isTransparent ? undefined : { background: config.background }}
    >
